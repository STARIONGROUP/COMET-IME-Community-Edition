// --------------------------------------------------------------------------------------------------------------------
// <copyright file="BinaryRelationshipDropTarget.cs" company="RHEA System S.A.">
...
namespace CDP4Composition.Diagram
{
    ...
    /// </summary>
    public class BinaryRelationshipDropTarget : IDropTarget
    {
        /// <summary>
        /// The NLog logger
        /// </summary>
        ...
        /// </summary>
        private readonly BinaryRelationship binaryRelationship;
        ...
        /// The <see cref="ISession"/>
        /// </param>
        public BinaryRelationshipDropTarget(BinaryRelationship binaryRelationship, ISession session)
        {
            this.binaryRelationship = binaryRelationship;
        ...
        {
            if (this.binaryRelationship.Category.Any(x => x.Equals(category)))
            {
                dropInfo.Effects = DragDropEffects.None;
                return;
            }

            if (this.session.OpenIterations.TryGetValue(this.binaryRelationship.GetContainerOfType<Iteration>(), out var tuple))
            {
                await this.thingCreator.ApplyCategory(category, this.binaryRelationship, this.session);
            }
        }
        ...
        private void CategoryDragOver(IDropInfo dropinfo, Category category)
        {
            ...
            var model = (EngineeringModel)this.binaryRelationship.TopContainer;
            ...
            if (!this.permissionService.CanWrite(this.binaryRelationship))
            {
                dropinfo.Effects = DragDropEffects.None;
            ...
            if (!category.PermissibleClass.Contains(ClassKind.BinaryRelationship))
            {
