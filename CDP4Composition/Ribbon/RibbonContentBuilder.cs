...
namespace CDP4Composition.Ribbon
{
    ...
    public class RibbonContentBuilder : IRibbonContentBuilder
    {
        ...
        public RibbonContentBuilder([ImportMany] IEnumerable<ExtendedRibbonPageCategory> categories,
        ...
        private void ProcessRibbonPageGroups(RibbonControl ribbon)
        {
            foreach (var ribbonPageGroup in groups)
            {
                ...
                if (ribbon.SelfCategories.Select(category => category.Pages.OfType<ExtendedRibbonPage>()
                        .FirstOrDefault(x => x.RegionName == containerRegionName)).FirstOrDefault() is ExtendedRibbonPage ribbonPage)
                {
                    var insertPosition = GetPositionForRibbonPageGroup(ribbonPage.Groups, ribbonPageGroup);
        ...
        {
            foreach (var ribbonPage in pages)
            {
                if (ribbonPage.IsInDefaultPageCategory)
                {
                    ...
                    {
                        var insertPosition = GetPositionForRibbonPage(defaultCategory.Pages, ribbonPage);
                ...
                else
                {
                    ...
                    var customPageCategory = categories.FirstOrDefault(cat => cat.Name == categoryName);

                    if (customPageCategory != null)
                    {
                        var insertPosition = GetPositionForRibbonPage(customPageCategory.Pages, ribbonPage);
        ...
        public void BuildAndAppendToRibbon(RibbonControl ribbon)
        {
            if (pages.Any())
            {
                ProcessRibbonPages(ribbon);
            }

            if (groups.Any())
            {
                ProcessRibbonPageGroups(ribbon);
            }

            if (categories.Any())
            {
                ProcessRibbonPageCategories(ribbon);
            }
        }
    }
}
