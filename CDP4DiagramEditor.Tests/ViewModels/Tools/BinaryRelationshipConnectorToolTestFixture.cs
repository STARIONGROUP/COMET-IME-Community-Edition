// --------------------------------------------------------------------------------------------------------------------
// <copyright file="BinaryRelationshipConnectorToolTestFixture.cs" company="RHEA System S.A.">
...
namespace CDP4DiagramEditor.Tests.ViewModels.Tools
{
    ...
    public class BinaryRelationshipConnectorToolTestFixture
    {
        [SetUp]
        public void Setup()
        {
            ...
            this.tool = new BinaryRelationshipConnectorTool();
            ...
            this.thingCreator.Setup(c => c.CreateAndGetBinaryRelationship(It.IsAny<ElementDefinition>(), It.IsAny<ElementDefinition>(), It.IsAny<Category>(), It.IsAny<Iteration>(), It.IsAny<DomainOfExpertise>(), this.session.Object)).Returns(Task.FromResult(this.binaryRelationship));
        }

        private BinaryRelationshipConnectorTool tool;
        ...
        {
            Assert.AreEqual("Binary Relationship Tool", this.tool.ToolName);
            Assert.AreEqual(nameof(BinaryRelationshipConnectorTool), this.tool.ToolId);
        ...
        public void VerifyConnectorToolWorksWhenItemsSet()
        {
            ...
            Assert.DoesNotThrowAsync(() => this.tool.ExecuteCreate(connector, this.behavior.Object));
            this.editor.Verify(e => e.ShowDropContextMenuOptions(It.IsAny<List<ContextMenuItemViewModel>>()), Times.Once);

            Assert.DoesNotThrowAsync(() => this.tool.CreateCallback(connector, this.behavior.Object, this.beginVm, this.endVm));
            this.behavior.Verify(x => x.ResetTool(), Times.Once);
