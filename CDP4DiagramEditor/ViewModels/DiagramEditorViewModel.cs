...
namespace CDP4DiagramEditor.ViewModels
{
    ...
    public class DiagramEditorViewModel : BrowserViewModelBase<DiagramCanvas>, IPanelViewModel, IDropTarget
    {
        ...
        public async Task Drop(IDropInfo dropInfo)
        {
            ...
            else if (droppedThing is ElementDefinition elementDefinition)
            {
                //this.CreateDiagramObject(new PortContainerDiagramContentItem(droppedThing), diagramDrop.DiagramDropPoint);          
            }
        ...
        {
            var teest = this.SelectedItems.SingleOrDefault(x => x.Thing.DepictedThing == depictedThing);
            if (teest is DiagramContentItem target && target?.Content is PortContainerDiagramContentItem container)
            {
                var row = this.DiagramObjectCollection.SingleOrDefault(x => x.Thing.DepictedThing == depictedThing);
        ...
        private void OnBinaryRelationshipDrop(BinaryRelationship binaryRelationship, System.Windows.Point dropPosition)
        {
            ...
            if (diagramObjectSource == null)
            {
                ...
                var binaryThingRelationship = binaryRelationship.Source as DiagramObject;
                diagramObjectSource = this.CreateDiagramObject(binaryRelationship.Source as DiagramObject, sourceObjectPosition);
            }
