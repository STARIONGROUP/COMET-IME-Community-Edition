    ...
    public class BinaryRelationshipConnectorTool : ConnectorTool, IConnectorTool
    {
        ...
        public async Task CreateCallback(DiagramConnector connector, ICdp4DiagramBehavior behavior, ThingDiagramContentItemViewModel beginItemContent, ThingDiagramContentItemViewModel endItemContent)
        {
            try
            {
                ...
                var binaryRelationship = await this.ThingCreator.CreateAndGetBinaryRelationship(beginItemContent.Thing, endItemContent.Thing, connector.CustomStyleId as Category, iteration, behavior.ViewModel.Session.QuerySelectedDomainOfExpertise(iteration), behavior.ViewModel.Session);
