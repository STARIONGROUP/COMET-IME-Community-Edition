...
namespace CDP4VandV.Rules
{
    ...
    public class VnVItemCompletenessRule : BuiltInRule
    {
        ...
        /// recorded result.
        /// </summary>
        private static readonly string[] ConcludedStatuses = { "Passed", "Failed", "Closed" };
        ...
        public override IEnumerable<RuleViolation> Verify(Iteration iteration)
        {
                         ...
                         .Where(requirement => !requirement.IsDeprecated && VandVCoverageQuery.IsVnVItem(requirement) && !VandVProcedureWriter.IsStep(requirement)))
            {
                ...
                if (ConcludedStatuses.Contains(status, StringComparer.OrdinalIgnoreCase) && IsBlank(item, "vnv_result"))
                {
                    defects.Add($"its status is '{status}' but no result was recorded");
                }
