// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MultiRelationshipBrowserViewModelTestFixture.cs" company="Starion Group S.A.">
...
namespace CDP4EngineeringModel.Tests.ViewModels
{
    ...
    [TestFixture]
    public class MultiRelationshipBrowserViewModelTestFixture
    {
        ...
        public void VerifyThatMultiRelationshipsAreAddedModifiedRemoved()
        {
            ...
            Assert.IsTrue(((MultiRelationshipRowViewModel)viewmodel.Relationships[0]).Name.Contains("EG"));
            ...
            Assert.IsTrue(((MultiRelationshipRowViewModel)viewmodel.Relationships[0]).Name.Contains("E4"));
        ...
        [Test]
        public void VerifyThatMultiRelationshipColumnsArePopulated()
        {
            var category = new Category(Guid.NewGuid(), this.cache, this.uri) { Name = "Grouping", ShortName = "GRP" };
            this.srdl.DefinedCategory.Add(category);

            var viewmodel = new MultiRelationshipBrowserViewModel(this.iteration, this.session.Object, this.thingDialogNavigationService.Object, this.panelNavigationService.Object, null, null);

            var relationship = new MultiRelationship(Guid.NewGuid(), this.cache, this.uri) { Owner = this.domain, Name = "multiName", ModifiedOn = DateTime.Now };
            relationship.RelatedThing.Add(this.elementDefinition1);
            relationship.RelatedThing.Add(this.elementDefinition2);
            ...
            // when named, only the name is shown (related things are in dedicated columns)
            Assert.That(row.Name, Is.EqualTo("multiName"));
            Assert.That(row.RelatedThings, Does.Contain("E1"));
            Assert.That(row.RelatedThings, Does.Contain("E2"));
            Assert.That(row.RelatedThingsKinds, Does.Contain(ClassKind.ElementDefinition.ToString()));
            Assert.That(row.OwnerShortName, Is.EqualTo("DMN"));
            Assert.That(row.Categories, Does.Contain("GRP"));
        }
        ...
        public void VerifyThatBrowserIsCreated()
        {
            ...
            Assert.That(viewmodel.Caption, Does.StartWith(MultiRelationshipBrowserViewModel.PanelCaptionText));
