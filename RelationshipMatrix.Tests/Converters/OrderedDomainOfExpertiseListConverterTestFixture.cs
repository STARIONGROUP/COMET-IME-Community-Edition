// --------------------------------------------------------------------------------------------------------------------
// <copyright file="OrderedDomainOfExpertiseListConverterTestFixture.cs" company="Starion Group S.A.">
...
namespace CDP4RelationshipMatrix.Tests.Converters
{
    ...
    public class OrderedDomainOfExpertiseListConverterTestFixture
    {
        ...
        [Test]
        public void Verify_That_when_ConvertBack_is_called_on_not_a_list_of_DomainOfExpertise_no_exceptions_thrown()
        {
            var things = new List<Thing>();

            Assert.DoesNotThrow(() =>
            {
                var result = this.orderedDomainOfExpertiseListConverter.ConvertBack(things, null, null, null) as List<DomainOfExpertise>;
