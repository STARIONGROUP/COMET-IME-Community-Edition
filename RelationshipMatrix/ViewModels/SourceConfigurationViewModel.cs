...
namespace CDP4RelationshipMatrix.ViewModels
{
    ...
    public class SourceConfigurationViewModel : MatrixConfigurationViewModelBase
    {
        ...
        public SourceConfigurationViewModel(ISession session, Iteration iteration, Action onUpdateAction, RelationshipMatrixPluginSettings settings) : base(session, iteration, onUpdateAction, settings)
        {
        ...
        private void PopulatePossibleCategories()
        {
            ...
            {
                var categories = this.ReferenceDataLibraries.SelectMany(x => x.DefinedCategory).Where(x => x.PermissibleClass.Contains(this.SelectedClassKind.Value)).OrderBy(x => x.Name);
                this.PossibleCategories.AddRange(categories);
            }
            else
            {
                var categories = this.ReferenceDataLibraries.SelectMany(x => x.DefinedCategory).Where(x => x.PermissibleClass.Intersect(this.PossibleClassKinds).Any()).OrderBy(x => x.Name);
                this.PossibleCategories.AddRange(categories);
            }
        }
    }
