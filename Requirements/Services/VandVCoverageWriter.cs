...
namespace CDP4Requirements.Services
{
    ...
    public class VandVCoverageWriter
    {
        ...
        private static void AddLink(Iteration iterationClone, IThingTransaction transaction, ReferenceDataLibrary mrdl, Thing source, Thing target, string categoryShortName, DomainOfExpertise owner)
        {
            ...
            if (category == null)
            {
                // silently dropping the link would report a successful write that did not record the coverage the
                // user asked for; the callers catch this and show it
                throw new InvalidOperationException($"The '{categoryShortName}' category was not found. Run 'Set up V&V' first.");
            }
